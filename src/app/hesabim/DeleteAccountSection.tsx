...
export default function DeleteAccountSection() {
  ...
  if (!open) {
    return (
      <div style={{ padding: 'clamp(20px, 4vw, 28px)', border: '1px solid rgba(209,123,106,0.25)', backgroundColor: 'rgba(209,123,106,0.04)' }}>
        <h3 style={{ fontFamily: 'var(--font-headline-lg)', color: 'var(--color-on-surface)', fontSize: '20px', fontWeight: 500, margin: '0 0 8px' }}>
          Hesabımı sil
        </h3>
        <p style={{ color: 'var(--color-on-surface-variant)', fontSize: '13px', lineHeight: 1.7, margin: '0 0 18px' }}>
  ...
  return (
    <div style={{ padding: 'clamp(20px, 4vw, 28px)', border: '1px solid var(--color-error)', backgroundColor: 'rgba(209,123,106,0.06)' }}>
      <p style={{ fontFamily: 'var(--font-label-spec)', fontSize: '10px', letterSpacing: '0.28em', color: 'var(--color-error)', textTransform: 'uppercase', margin: '0 0 12px' }}>
        Son Onay
      </p>
      <h3 style={{ fontFamily: 'var(--font-headline-lg)', color: 'var(--color-on-surface)', fontSize: '22px', fontWeight: 500, margin: '0 0 12px' }}>
        Bu işlem geri alınamaz.
      </h3>
      <p style={{ color: '#D4CFC2', fontSize: '13px', lineHeight: 1.7, margin: '0 0 20px' }}>
        Devam etmek için <strong style={{ color: 'var(--color-on-surface)' }}>şifrenizi</strong> ve onay metnini girin.
      </p>
      ...
      <div style={{ marginBottom: '14px' }}>
        <label style={labelStyle}>
          Onay için <code style={{ background: 'var(--color-hairline-light)', padding: '1px 6px', fontFamily: 'var(--font-label-spec)' }}>{CONFIRM_TEXT}</code> yazın
        </label>
        <input
          className="auth-input"
          type="text"
