      ...
      <main style={{ background: 'var(--color-surface)', minHeight: '70vh' }}>
        ...
        >
          <div className="px-margin" style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
            <Link href="/" style={{ fontFamily: 'var(--font-label-spec)', fontSize: '10px', letterSpacing: '0.18em', color: 'var(--color-outline)', textTransform: 'uppercase', textDecoration: 'none' }}>
              Anasayfa
            </Link>
            <span style={{ color: 'var(--color-hairline-light)', fontSize: '10px' }}>·</span>
            <span style={{ fontFamily: 'var(--font-label-spec)', fontSize: '10px', letterSpacing: '0.18em', color: 'var(--color-honey-amber)', textTransform: 'uppercase' }}>
              Sipariş Takibi
            </span>
          </div>
        </nav>
        ...
        <section style={{ paddingTop: 'clamp(48px, 8vw, 80px)', paddingBottom: 'clamp(24px, 5vw, 48px)' }}>
          <div className="px-margin" style={{ maxWidth: '720px', margin: '0 auto', textAlign: 'center' }}>
            <p style={{ fontFamily: 'var(--font-label-spec)', fontSize: 'clamp(10px, 2vw, 11px)', letterSpacing: '0.3em', color: 'var(--color-honey-amber)', textTransform: 'uppercase', margin: '0 0 20px' }}>
              Yardım · Sipariş Takibi
            </p>
            <div style={{ width: '60px', height: '1px', backgroundColor: 'var(--color-charcoal-pure)', margin: '0 auto 32px' }} />
            <h1 className="font-headline-lg" style={{ color: 'var(--color-on-surface)', fontSize: 'clamp(34px, 6vw, 56px)', fontWeight: 500, lineHeight: 1.05, letterSpacing: '-0.015em', margin: 0 }}>
              Siparişiniz{' '}
              <span style={{ fontStyle: 'italic', fontWeight: 300, color: 'var(--color-on-surface-variant)' }}>şimdi nerede?</span>
            </h1>
            <p style={{ color: 'var(--color-on-surface-variant)', fontSize: 'clamp(14px, 2.5vw, 16px)', lineHeight: 1.7, marginTop: '24px', maxWidth: '480px', margin: '24px auto 0' }}>
        ...
        <section style={{ paddingBottom: 'clamp(40px, 6vw, 64px)' }}>
          <div className="px-margin" style={{ maxWidth: '520px', margin: '0 auto' }}>
            <form
              method="get"
              style={{
            ...
            >
              <div style={{ marginBottom: '18px' }}>
                ...
                <input
                  ...
                  spellCheck={false}
                  style={{ ...inputStyle, textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: 'var(--font-label-spec)' }}
                />
                <p style={hintStyle}>
                  Sipariş onay sayfasındaki <strong style={{ color: 'var(--color-honey-amber)' }}>DS-YYYY-NNNN</strong> formatlı numara.
                </p>
              </div>
        ...
        {result && (
          <section style={{ paddingBottom: 'clamp(80px, 10vw, 120px)' }}>
            <div className="px-margin" style={{ maxWidth: '880px', margin: '0 auto' }}>
...
const inputStyle: React.CSSProperties = {
  ...
  color: 'var(--color-on-surface)',
  fontSize: '14px',
  fontFamily: 'var(--font-body-md), system-ui, sans-serif',
  outline: 'none',
  borderRadius: 0,
  minHeight: '44px',
}
  ...
  return (
    <div>
      ...
      <div style={{ textAlign: 'center', marginBottom: '32px' }}>
        <p style={{ fontFamily: 'var(--font-label-spec)', fontSize: '10px', letterSpacing: '0.3em', color: 'var(--color-success)', textTransform: 'uppercase', margin: '0 0 12px' }}>
          ✓ Sipariş bulundu
        </p>
        <h2 className="font-headline-lg" style={{ color: 'var(--color-on-surface)', fontSize: 'clamp(24px, 4vw, 36px)', fontWeight: 500, lineHeight: 1.1, margin: 0 }}>
          {order.order_number}
        </h2>
      ...
      {/* Status timeline */}
      {!isCancelled ? (
        <div style={{ background: 'var(--color-surface-container-low)', border: '1px solid var(--color-hairline-light)', padding: 'clamp(20px, 4vw, 32px)', marginBottom: '24px' }}>
          <p style={{ fontFamily: 'var(--font-label-spec)', fontSize: '10px', letterSpacing: '0.3em', color: 'var(--color-honey-amber)', textTransform: 'uppercase', margin: '0 0 24px' }}>
            Durum
          </p>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <span
                      ...
                      style={{
                        ...
                        height: '24px',
                        borderRadius: '50%',
                        background: done ? 'var(--color-honey-amber)' : 'transparent',
                        border: done ? 'none' : '1px solid var(--color-hairline-light)',
                        display: 'inline-flex',
                        alignItems: 'center',
                    ...
                    {i < STATUS_TIMELINE.length - 1 && (
                      <span
                        ...
                        style={{
                          flex: 1,
                          height: '1px',
                          background: i < curIdx ? 'var(--color-honey-amber)' : 'var(--color-hairline-light)',
                        }}
                      />
                    )}
                  </div>
                  <div style={{ marginTop: '10px', paddingRight: '8px' }}>
                    <p style={{ fontFamily: 'var(--font-headline-lg)', color: isCurrent ? 'var(--color-on-surface)' : done ? 'var(--color-on-surface-variant)' : 'var(--color-outline)', fontSize: '15px', fontWeight: 500, margin: 0, lineHeight: 1.2 }}>
                      {s.label}
                    </p>
                    <p style={{ fontFamily: 'var(--font-label-spec)', fontSize: '10px', color: 'var(--color-outline)', letterSpacing: '0.05em', marginTop: '4px', lineHeight: 1.4 }}>
                      {s.description}
                    </p>
                  </div>
                </li>
          ...
          {order.tracking_number && (order.status === 'shipped' || order.status === 'delivered') && (
            <div style={{ marginTop: '28px', padding: '14px 16px', background: 'var(--color-hairline-light)', border: '1px solid var(--color-hairline-light)' }}>
              <p style={{ fontFamily: 'var(--font-label-spec)', fontSize: '10px', letterSpacing: '0.22em', color: 'var(--color-honey-amber)', textTransform: 'uppercase', margin: '0 0 4px' }}>
                Kargo Takip No
              </p>
              <p className="font-label-spec" style={{ color: 'var(--color-on-surface)', fontSize: '14px', margin: 0, letterSpacing: '0.05em', wordBreak: 'break-all' }}>
        ...
        <div style={{ background: 'rgba(200,71,45,0.06)', border: '1px solid rgba(200,71,45,0.3)', padding: 'clamp(20px, 4vw, 32px)', marginBottom: '24px', textAlign: 'center' }}>
          <p style={{ fontFamily: 'var(--font-label-spec)', fontSize: '11px', letterSpacing: '0.3em', color: 'var(--color-error)', textTransform: 'uppercase', margin: '0 0 10px' }}>
      ...
      <div style={{ background: 'var(--color-surface-container-low)', border: '1px solid var(--color-hairline-light)', padding: 'clamp(20px, 4vw, 32px)', marginBottom: '24px' }}>
        ...
        {items.map((it) => (
          <div key={it.id} style={{ display: 'flex', gap: '14px', paddingBottom: '16px', marginBottom: '16px', borderBottom: '1px solid var(--color-hairline-light)' }}>
            <div style={{ width: '64px', height: '80px', flexShrink: 0, background: 'var(--color-surface)', position: 'relative' }}>
              {it.product_image ? (
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <p className="font-headline-lg" style={{ color: 'var(--color-on-surface)', fontSize: '16px', fontWeight: 500, lineHeight: 1.25, margin: 0 }}>
                {it.product_name}
              </p>
              {it.variant_label && (
                <p className="font-label-spec" style={{ fontSize: '9.5px', letterSpacing: '0.18em', color: 'var(--color-outline)', textTransform: 'uppercase', margin: '4px 0 0' }}>
                  {it.variant_label}
                </p>
              )}
              <p className="font-label-spec" style={{ fontSize: '11px', color: 'var(--color-on-surface-variant)', margin: '6px 0 0' }}>
                {it.quantity} × {formatPrice(Number(it.unit_price))}
              </p>
            </div>
            <div style={{ flexShrink: 0, textAlign: 'right' }}>
              <p className="font-headline-lg" style={{ fontSize: '17px', fontWeight: 500, color: 'var(--color-on-surface)', margin: 0 }}>
        ...
        <div style={{ paddingTop: '12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
          ...
          <div style={{ borderTop: '1px solid var(--color-hairline-light)', marginTop: '8px', paddingTop: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
            <span className="font-label-spec" style={{ fontSize: '11px', letterSpacing: '0.22em', color: 'var(--color-on-surface)', textTransform: 'uppercase' }}>Toplam</span>
            <span className="font-headline-lg" style={{ color: 'var(--color-honey-amber)', fontSize: '24px', fontWeight: 500 }}>
      ...
      {ship && (
        <div style={{ background: 'var(--color-surface-container-low)', border: '1px solid var(--color-hairline-light)', padding: 'clamp(20px, 4vw, 32px)', marginBottom: '24px' }}>
          ...
          <p style={{ color: 'var(--color-on-surface)', fontSize: '14px', lineHeight: 1.7, margin: 0 }}>
            {ship.full_name}<br />
            {ship.address_line1}{ship.address_line2 ? `, ${ship.address_line2}` : ''}<br />
            {ship.district}, {ship.city}{ship.postal_code ? ` ${ship.postal_code}` : ''}<br />
            <span className="font-label-spec" style={{ color: '#9B9285', fontSize: '12px' }}>{ship.phone}</span>
          </p>
        </div>
      )}
...
function SumRow({ label, value, muted, highlight }: { label: string; value: string; muted?: boolean; highlight?: boolean }) {
  return (
    <div className="font-label-spec" style={{ display: 'flex', justifyContent: 'space-between', fontSize: muted ? '11px' : '12px', color: muted ? 'var(--color-outline)' : 'var(--color-on-surface-variant)' }}>
      <span>{label}</span>
